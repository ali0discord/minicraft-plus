...
//import static minicraft.core.*;

public class Initializer extends minicraft.core.Game {
	private Initializer() {}
	...
	static void parseArgs(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i].equalsIgnoreCase("--debug")) {
				...
				minicraft.core.Updater.FULLSCREEN = true;
	...
	static void run() {
		...
		while (running) {
			...
			double nsPerTick = 1E9D / minicraft.core.Updater.normSpeed; // Nanosecs per sec divided by ticks per sec = nanosecs per tick
			if (display == null) nsPerTick /= minicraft.core.Updater.gamespeed;
			...
			while (unprocessed >= 1) { // If there is unprocessed time, then tick.
				ticks++;
				minicraft.core.Updater.tick(); // Calls the tick method (in which it calls the other tick methods throughout the code.
				unprocessed--;
			}
			...
			if ((now - lastRender) / 1.0E9 > 1.0 / MAX_FPS) {
				...
				minicraft.core.Renderer.render();
			}
	...
	static void createAndDisplayFrame() {
		minicraft.core.Renderer.canvas.setMinimumSize(new java.awt.Dimension(1, 1));
		minicraft.core.Renderer.canvas.setPreferredSize(minicraft.core.Renderer.getWindowSize());
		JFrame frame = Initializer.frame = new JFrame(NAME);
		frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
		frame.setLayout(new BorderLayout()); // Sets the layout of the window
		frame.add(minicraft.core.Renderer.canvas, BorderLayout.CENTER); // Adds the game (which is a canvas) to the center of the screen.
		frame.pack(); // Squishes everything into the preferredSize.

		try {
			BufferedImage logo = ImageIO.read(minicraft.core.Game.class.getResourceAsStream("/resources/logo.png")); // Load the window logo
		...
		frame.addComponentListener(new ComponentAdapter() {
			public void componentResized(ComponentEvent e) {
				...
				minicraft.core.Renderer.SCALE = Math.min(w / minicraft.core.Renderer.WIDTH, h / minicraft.core.Renderer.HEIGHT);
			}
		});
